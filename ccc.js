...
(function ArabsModderThemeScript() {
        ...
        if (firebaseUrl) {
            ...
            firebaseScript.onload = () => {
                ...
                firebaseDbScript.onload = () => {
                    ...
                    const views = document.querySelectorAll('.Ar-get-view');
                    views.forEach(view => {
        ...
        paginationElements.forEach(paginationElement => {
            ...
            window.jo[callbackName] = function (response) {
                ...
                if (paginationData.currentPage > 1) {
                    ...
                    prevPage.innerHTML = `<span class="btn btn-sm rounded-pill Ar-icon-center" data-page="${paginationData.currentPage - 1}"><svg aria-hidden="true" class="Ar-icon"><use xlink:href="#i-arrow-l"/></svg></span>`;
                    paginationContainer.appendChild(prevPage);
                }

                paginationData.pages.forEach(page => {
                    const pageElement = document.createElement('li');
                    pageElement.innerHTML = `<span class="btn btn-sm rounded-pill Ar-icon-center" data-page="${page}">${page}</span>`;
                    paginationContainer.appendChild(pageElement);
                });

                if (paginationData.currentPage < paginationData.totalPages) {
                    const nextPage = document.createElement('li');
                    nextPage.innerHTML = `<span class="btn btn-sm rounded-pill Ar-icon-center" data-page="${paginationData.currentPage + 1}"><svg aria-hidden="true" class="Ar-icon"><use xlink:href="#i-arrow-r"/></svg></span>`;
